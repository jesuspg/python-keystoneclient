...
class KeyManager(base.Manager):
    ...
    def _base_url(self, user):
        return self._url(user) + self.base_url
    ...
    def deactivate_two_factor(self, user):
        return super(KeyManager, self)._delete(url=self._base_url(user))

    def check_activated_two_factor(self, user_id=None, username=None, domain_id=None, domain_name=None):
        try:
            if user_id:
                super(KeyManager, self)._head(url=EXTENSION_PATH+self.base_url+'?user_id='+user_id)
            elif domain_id:
                super(KeyManager, self)._head(url=EXTENSION_PATH+self.base_url+'?user_name='+username+'&domain_id='+domain_id)
            elif domain_name:
                super(KeyManager, self)._head(url=EXTENSION_PATH+self.base_url+'?user_name='+username+'&domain_name='+domain_name)
            return True
        except:
            return False

    def get_two_factor_data(self, user):
        return super(KeyManager, self)._get(url=self._url(user)+self.two_factor_data_url,
                                            response_key="two_factor_auth")
    ...
    def check_security_question(self, user, security_answer):
        ...
        return super(KeyManager, self)._head(body=data,
                                            url=self._url(user)+self.security_question_url)
